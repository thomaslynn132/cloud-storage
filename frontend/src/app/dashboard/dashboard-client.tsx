...
export default function DashboardClient({ initialFiles, initialStorage, initialProfile }: DashboardClientProps) {
  ...
  useKeyShortcuts({
    u: () => router.push('/upload'),
    'ctrl+f': () => searchRef.current?.focus(),
    Escape: () => { setRenaming(null); setSelected(new Set()); },
  });

  const loadData = async () => {
    const [filesData, storageData] = await Promise.all([
      fileService.getFiles() as Promise<FileData[]>,
      api.get('/users/storage').then((r: { data: StorageInfo }) => r.data),
    ]);
    setFiles(filesData);
  ...
  const handleBulkDelete = async () => {
    ...
    for (const id of selected) {
      try { await fileService.deleteFile(id); success++; } catch { /* skip */ }
    }
    setSelected(new Set());
  ...
  const sortedFiles = [...filteredFiles].sort((a: FileData, b: FileData) => {
    if (sortBy === 'name') return a.fileName.localeCompare(b.fileName);
    if (sortBy === 'downloads') return b.downloadCount - a.downloadCount;
    return new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime();
  });

  const totalDownloads = files.reduce((sum: number, f: FileData) => sum + f.downloadCount, 0);
  const expiringSoon = files.filter((f: FileData) =>
      ...
      <nav className="bg-white shadow">
        <div className="max-w-7xl mx-auto px-4 py-3">
          <div className="flex justify-between items-center">
            <h1 className="text-xl md:text-2xl font-bold">Uploader Panel</h1>
            ...
            <div className="hidden md:flex items-center gap-3">
              <span className="text-sm text-gray-500 truncate max-w-[160px]">{profile?.email}</span>
          ...
          {mobileMenuOpen && (
            <div className="md:hidden mt-3 pt-3 border-t flex flex-col gap-2">
              ...
              <Button variant="link" asChild className="justify-start px-0"><Link href="/upload">Upload</Link></Button>
      ...
      <main className="max-w-7xl mx-auto p-4 md:p-8">
        <div className="grid gap-4 grid-cols-2 md:grid-cols-4 mb-8">
          <Card className="scale-in"><CardContent className="pt-4 md:pt-6 text-center">
            ...
            <p className="text-xs md:text-sm text-gray-500">Total Files</p>
          </CardContent></Card>
          <Card className="scale-in" style={{ animationDelay: '0.05s' }}><CardContent className="pt-4 md:pt-6 text-center">
            <p className="text-xl md:text-2xl font-bold">{formatSize(storageInfo.used)}</p>
            <p className="text-xs md:text-sm text-gray-500">Storage Used</p>
          </CardContent></Card>
          ...
          <Card className="scale-in" style={{ animationDelay: '0.15s' }}><CardContent className="pt-4 md:pt-6 text-center">
            <p className="text-xl md:text-2xl font-bold">{expiringSoon.length}</p>
        ...
        <Card className="fade-slide-in" style={{ animationDelay: '0.2s' }}>
          <CardHeader>
              ...
              <>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {sortedFiles.map((file: FileData, i: number) => (
                        <tr key={file.id} className="border-b last:border-0 hover:bg-gray-50 row-fade-in" style={{ animationDelay: `${i * 0.03}s` }}>
                          ...
                          <td className="py-3">
                            <div className="flex gap-1 flex-wrap">
                              <Button variant="outline" size="sm" onClick={() => startRename(file)}>Rename</Button>
                              <Button variant="outline" size="sm" onClick={() => handleCopyLink(file.id)} className={copiedId === file.id ? 'copy-bounce' : ''}>
